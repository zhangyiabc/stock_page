...
- **审校时，正文和大纲通过 task 的 context 传入**，直接从 context 中读取即可
- **不要尝试从文件系统中查找正文文件**。正文存储在 coordinator 的 workspace 下，你无法直接访问
- 如需补充角色或世界设定信息，使用 `memory_search` 搜索，不要用 `read` 去猜测文件路径
